...
const DashboardPage = () => {
	...
	const updatePostDatedVoucher = (id) => {
		...
		Axios.post(
			`${baseURL}/clearPostDatedVoucher?voucher_id=${id}&is_post_dated=${postDatedStatusOptionsSelected}&date=${postDatedDateStatusOptionsSelected}`,
		)
			.then((response) => {
	...
	const refreshVouchers = () => {
		...
		const getVouchersByType = () => {
			Axios.get(
				`${baseURL}/getVouchers?type=${voucherType === '0' ? '' : voucherType}
	...
	const getVoucherHistory = (id) => {
		setEditingVoucherDataLoading(true);
		Axios.get(`${baseURL}/getEditedVoucherDetails?voucher_id=${id}`)
			.then((response) => {
				setEditingVoucherData(response.data.voucher);
