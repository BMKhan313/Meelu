...
const DashboardPage = () => {
	...
	useEffect(() => {
		const getAllEntriesExtendedLedger = () => {
			...
			// Axios.get(`${baseURL}/getDailyClosingReport?date=${dailyClosingDate}`)
			Axios.get(
				`${baseURL}/getAccountLedger?account_id=${accountSelected?.id}&from=${dailyClosingDate}&to=${dailyClosingDate}`,
			)
				.then((response) => {
					setOpeningBalance(
