...
const DashboardPage = (props) => {
	...
	useEffect(() => {
		const getFilesByAccount = () => {
			...
			Axios.get(`${baseURL}/getFilesByAccount?account_id=${props.accountSelected.id}`)
				.then((response) => {
		...
		const getPaymentHeadsByFileAndAccount = () => {
			Axios.get(
				// eslint-disable-next-line react/destructuring-assignment
				`${baseURL}/getPaymentHeadsByFileAndAccount?account_id=${props.accountSelected.id}`,
			)
				.then((response) => {
	...
	const searchFilterTrigger = () => {
		...
		const getAllEntries = () => {
			...
			Axios.get(
				// eslint-disable-next-line react/destructuring-assignment
				`${baseURL}/getAccountLedger?account_id=${props.accountSelected.id}&from=${startDate}&to=${endDate}`,
			)
				.then((response) => {
					setOpeningBalance(
