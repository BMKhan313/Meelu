...
const GeneratePDF = (data1, type, startDate, endDate) => {
	...
	let crTotal = 0;
	let costTotal = 0;
	const CalcluateTotalCost = (amount) => {
		costTotal += parseFloat(Math.abs(amount));
	};
	...
	if (data1.revenues !== undefined) {
		...
		const TableData4 = [
			...
			{
				content: `${Math.abs(crTotal).toLocaleString(undefined, {
					maximumFractionDigits: 2,
				})}`,
	...
	if (data1.expenses !== undefined) {
		...
		const TableData4 = [
			...
			{
				content: `${Math.abs(drTotal).toLocaleString(undefined, {
					maximumFractionDigits: 2,
				})}`,
	...
	if (data1.cost !== undefined) {
						...
						itemSubGroups.coa_accounts.forEach((itemAccounts) => {
							...
							if (itemAccounts.balance !== null) {
								if (itemAccounts.balance !== null) {
									...
									CalcluateTotalCost(itemAccounts.balance.balance);
								}
								rowsMain.push(TableData3);
							}
		...
		const TableData4 = [
			...
			{
				content: `${Math.abs(costTotal).toLocaleString(undefined, {
					maximumFractionDigits: 2,
				})}`,
	...
	const TableDataTotal = [
		{
			content: `${crTotal - drTotal - costTotal > 0 ? `Net Profit` : `Net Loss`}`,
			colSpan: 1,
			rowSpan: 1,
			styles: {
				halign: 'left',
		...
		{
			content: `${
				crTotal - drTotal - costTotal > 0
					? `${Math.abs(crTotal - drTotal - costTotal).toLocaleString(undefined, {
							maximumFractionDigits: 2,
					  })}`
					: `(${Math.abs(crTotal - drTotal - costTotal).toLocaleString(undefined, {
							maximumFractionDigits: 2,
