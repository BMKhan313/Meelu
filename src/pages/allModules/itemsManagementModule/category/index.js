...
const Categories = () => {
	...
	const refreshTableData = () => {
		...
		Axios.get(
			`${baseURL}/getCategories?records=${store.data.itemsManagementModule.category.perPage}&pageNo=${store.data.itemsManagementModule.category.pageNo}&colName=id&sort=desc`,
			{},
		)
			.then((response) => {
				console.log('getcategory', response.data);
				console.log('getcategory', response.data.categories.data[0].name);
