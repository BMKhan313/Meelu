...
const List = () => {
	...
	const refreshTableData = () => {
		...
		Axios.get(
			`${baseURL}/getSupplierLedger?supplier_id=${selectedSupplier.id}&records=${
				store.data.suppliersManagementModule.ledger.perPage
			}&pageNo=${
				store.data.suppliersManagementModule.ledger.pageNo
			}&colName=id&sort=asc&from_date=${fromDate}&${toDate !== '' && `&to_date=${toDate}`}`,
		)
			.then((response) => {
	...
	useEffect(() => {
		Axios.get(`${baseURL}/getSuppliersDropDown`)
			.then((response) => {
				const rec = response.data.suppliersManagementModule.map(({ id, name }) => ({
					id,
					value: id,
					label: name,
				}));
